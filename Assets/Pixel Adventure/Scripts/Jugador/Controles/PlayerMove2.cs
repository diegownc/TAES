...
public class PlayerMove2 : MonoBehaviour
{
    ...
    private void MovimientoHorizontal()
    {
        ...
        if (horizontalInput > 0.01 || horizontalInput < -0.01)
        {
            ...
            if (!(horizontalInput < -0.01 && CheckLeft2.isLeft) && !(horizontalInput > 0.01 && CheckRight2.isRight))
    ...
    private void PlayerSpawn()
    {
        ...
        {
            spawn = false;
            GameObject.Find("SpawnPlayerMascara").GetComponent<SpawnPlayer>().Spawn();
        }
    }

    private void ResetCheck()
    {
        CheckGround2.isGround = false;
        CheckUp2.isUp = false;
        CheckLeft2.isLeft = false;
        CheckRight2.isRight = false;
    }
