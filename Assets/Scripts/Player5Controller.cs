...
public class Player5Controller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        gameObject.GetComponent<Transform>().position = new Vector3(-480, -250, 0);

        udp = new UdpClient(8060);
    ...
    void Update()
    {
        if (gameObject.transform.position.x < 750)
        {
            ...
            {
                if ((Input.GetKey(KeyCode.G) || push) && veces == 0)
                {
                    push = false;
